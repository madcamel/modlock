...
| `modlock --disable`   | Set `/proc/sys/kernel/modprobe` = `/bin/true` |
| `modlock --enable`    | Set `/proc/sys/kernel/modprobe` back to the real modprobe |
...
Anything that wants a new module will fail until you re-enable:

```
 modlock --enable     # autoload back on
# plug / install / configure the new thing
 modlock --update     # capture the expanded module set
 modlock --disable    # gate shut until reboot
```

`--install` only needs to run once. `--update` + `--disable` is enough thereafter.
